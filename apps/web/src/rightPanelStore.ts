...
export type RightPanelSurface =
  ...
  /**
   * A side chat: its own thread, opened against a selection in this thread.
   * The quotation rides on the surface rather than the transcript so the panel
   * can show it before a single word has been asked. Absent on surfaces
   * persisted before quotes were kept.
   */
  | { id: `chat:${string}`; kind: "chat"; resourceId: string; title: string; quote?: string }
  | {
      /**
...
interface RightPanelStoreState {
  ...
  openSideChat: (ref: ScopedThreadRef, threadId: string, title: string, quote: string) => void;
  splitTerminal: (
    ref: ScopedThreadRef,
    surfaceId: string,
    terminalId: string,
...
const chatSurface = (threadId: string, title: string, quote: string): RightPanelSurface => ({
  id: `chat:${threadId}`,
  kind: "chat",
  resourceId: threadId,
  title,
...
export const useRightPanelStore = create<RightPanelStoreState>()(
  persist(
    (set) => ({
      ...
      openSideChat: (ref, threadId, title, quote) =>
        set((state) => ({
          byThreadKey: updateThread(state.byThreadKey, scopedThreadKey(ref), (current) =>
            upsertSurface(current, chatSurface(threadId, title, quote)),
          ),
        })),
