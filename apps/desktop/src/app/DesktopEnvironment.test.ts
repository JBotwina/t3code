...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, `${FORK_BRAND.appIdDev}`);
      assert.equal(environment.linuxWmClass, `${FORK_BRAND.schemeDev}`);
      assert.deepEqual(
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, `/Users/alice/${FORK_BRAND.homeDirName}/dev`);
      assert.equal(production.stateDir, `/Users/alice/${FORK_BRAND.homeDirName}/userdata`);
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " io.jamesbotwina.t2code.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "io.jamesbotwina.t2code.dev.local");
    }),
  );
