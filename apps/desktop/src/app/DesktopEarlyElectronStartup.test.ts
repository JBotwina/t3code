...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("resolves the early linux Electron switches", () => {
    ...
    assert.deepEqual(options, {
      linuxWmClass: `${FORK_BRAND.schemeDev}`,
      passwordStore: "gnome-libsecret",
    });
  });

  it(`keeps implicit development state under ~/${FORK_BRAND.homeDirName}/dev when T3CODE_HOME is unset`, () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, `/home/user/${FORK_BRAND.homeDirName}/dev/desktop-settings.json`);
