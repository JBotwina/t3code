...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: `${FORK_BRAND.baseName} (Alpha)`,
    linuxWmClass: `${FORK_BRAND.scheme}`,
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      displayName: `${FORK_BRAND.baseName} (Nightly)`,
      execTarget: '/home/al ice/Apps/T3 "100%" $HOME\\x.AppImage',
      scheme: `${FORK_BRAND.scheme}`,
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, `Name=${FORK_BRAND.baseName} (Nightly)`);
    ...
    assert.include(entry, `MimeType=x-scheme-handler/${FORK_BRAND.scheme};`);
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: `${FORK_BRAND.scheme}`,
      desktopEntryPath: `/home/alice/.local/share/applications/${FORK_BRAND.scheme}-url-handler.desktop`,
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      `Failed to register the ${FORK_BRAND.scheme}:// URL handler (step: write-desktop-entry).`,
    );
    assert.equal(
      writeError.desktopEntryPath,
      `/home/alice/.local/share/applications/${FORK_BRAND.scheme}-url-handler.desktop`,
    );
    ...
    assert.equal(
      exitError.message,
      `Failed to register the ${FORK_BRAND.scheme}:// URL handler (step: set-default-handler, xdg-mime exit code 4).`,
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(recorded.files[0]?.content, `MimeType=x-scheme-handler/${FORK_BRAND.scheme};`);
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: [
            "default",
            `${FORK_BRAND.scheme}-url-handler.desktop`,
            `x-scheme-handler/${FORK_BRAND.scheme}`,
          ],
        },
      ]);
    });
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: `/home/alice/.local/share/applications/${FORK_BRAND.scheme}-url-handler.desktop`,
        }),
      });
